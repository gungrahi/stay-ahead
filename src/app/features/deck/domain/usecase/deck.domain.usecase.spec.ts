...
import { DeckParam, GettingDeckUsecase } from "./deck.domain.usecase";
...
describe("GettingDeckUsecase tests:", () => {
  ...
  it("should execute itself to get Deck", async () => {
    ...
    // act
    const resultEither = await usecase.execute(new DeckParam("id"));
    // assert
    expect(usecase.execute).toHaveBeenCalledWith(new DeckParam("id"));
    expect(repository.getDeckData).toHaveBeenCalledWith("id");
